...
def get_stock_values(data):
    """
    build a dictionary, where the keys are the sandwich headings pulled from the spreadsheet, and the values are the calculated stock data.
    """
    #data = SHEET.worksheet("stock").get_all_values()
    #headings = data[0]
    
    headings = SHEET.worksheet("stock").row_values(1)
    data = SHEET.worksheet("stock").get_all_values()
    vals = data[len(data)-1]
    
    stock_dict = dict()
    for header, val in zip(headings, vals):
        stock_dict[header] = int(val)
